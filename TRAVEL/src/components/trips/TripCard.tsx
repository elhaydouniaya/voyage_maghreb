        ...
        <div>
          <div className="flex flex-col lg:flex-row justify-between items-start gap-8 mb-10">
            ...
            <div className="lg:text-right bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm min-w-[180px]">
              <div className="text-4xl font-black text-orange-600 tracking-tighter leading-none mb-1">
                {formatPriceShort(trip.totalPrice)}
              </div>
              <div className="text-[9px] font-bold text-gray-400">
                (~{Math.round(trip.totalPrice)} €)
              </div>
          ...
          <div className="flex flex-wrap gap-3 mb-12">
            {trip.inclusions?.map((item: string, i: number) => (
              <div
                key={i}
                className="flex items-center gap-2 bg-white px-5 py-3 rounded-2xl border border-gray-100 shadow-sm"
              >
        ...
        <div className="flex flex-col xl:flex-row items-center gap-10 pt-10 border-t border-gray-100">
          <div className="flex-1 w-full space-y-4">
            <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.2em]">
              ...
              <span
                className={`${isAlmostFull ? "text-orange-600" : "text-gray-400"} bg-white px-4 py-1.5 rounded-full border border-gray-100 shadow-sm`}
              >
            ...
            <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden p-0.5">
              <div
                ref={(el) => {
                  if (el) el.style.width = `${fillingPercentage}%`;
                }}
